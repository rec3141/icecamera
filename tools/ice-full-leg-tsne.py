...
def worker(a):
    ...
    valid_files=set()
    if getattr(a,'base_portal',None):
        with sqlite3.connect(a.base_portal.joinpath('features.sqlite').resolve().as_uri()+'?mode=ro',uri=True) as base:
            for file,fingerprint,vector,raw,error in base.execute('SELECT file,fingerprint,vector,point,error FROM features WHERE error IS NULL'):
                point=json.loads(raw)
                point['images']=[(a.base_portal/p).resolve().as_uri() for p in point['images']]
                db.execute('INSERT OR IGNORE INTO features VALUES (?,?,?,?,?)',(file,fingerprint,vector,json.dumps(point),error))
                valid_files.add(file)
        db.commit()
    paths=[a.source/r['file'] for r in json.loads(a.queue.read_text())['queue']] if getattr(a,'queue',None) else sorted(a.source.glob('????????/*/Camera360_*_cam_3.jpg'))
    ...
    for i,path in enumerate(paths):
        ...
        try:
            ...
            point=dict(id=identity,file=relative,brightness=float(vector[21]),
                mean_rgb=[float(vector[j]) for j in (0,7,14)],
...
def main():
    ...
    p.add_argument('--queue',type=Path,help='Explicit source-relative photo queue')
    p.add_argument('--base-portal',type=Path,help='Reuse reference features in a separate combined output')
