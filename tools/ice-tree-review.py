...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--portal',type=Path,required=True);p.add_argument('--reset-labels',action='store_true');a=p.parse_args()
    ...
    reset_file=a.portal/'tree-label-reset.json'
    if a.reset_labels:reset_file.write_text(json.dumps({'token':uuid.uuid4().hex}))
    reset=json.loads(reset_file.read_text())['token'] if reset_file.exists() else None
    data=dict(points=points,tree=tree,roots=sorted(active),reset_token=reset)
...
SCRIPT</script>'''.replace('PAYLOAD',payload).replace('SCRIPT',js)
    page=page.replace('Enter: save and advance.','Enter: next pending group on this page; save only inside the label dialog.').replace('Save and next','Save').replace('L: label entire displayed batch.','L or .: label entire displayed batch.').replace('L · Label batch','L / . · Label batch').replace('Down: split one level.','Down or -: split one level.').replace('Up: parent','Up or +: parent')
